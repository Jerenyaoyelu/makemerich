...
- **历史某日截面**：按指定交易日用日线重建全市场快照（用于回测 T+1/T+2… 与复评中心联调）
...
| 页面 | 用途 |
|------|------|
| **main（实时选股）** | 最新行情：可「刷新实时数据」或「运行筛选」现采现算 |
| **历史回测 T0** | 指定过去交易日：只需 **运行历史回测筛选**，勿点实时页的刷新 |
| **复评中心** | 对任意 run 做多周期复评 |
| **参数实验室** | 权重回放与写回默认权重 |

## 闭环（run + 复评 + 权重实验）

- 在 **实时选股** 或 **历史回测 T0** 点击运行后都会生成 `run_id`，并写入 `data/runs.csv` 与 `data/run_candidates/{run_id}.csv`
- **复评中心**：选择 run →「更新复评数据」→ `data/run_evaluations/{run_id}.csv`（多周期脚本见下）
- **参数实验室**：对已复评的多个 run 做权重回放，可将推荐权重保存为 `data/ui_default_weights.json`（各页侧边栏滑块默认值）

## 目录结构

- `app/main.py`：实时选股入口
- `app/ui_screener_shared.py`：两页共用的展示与 run 收尾逻辑
- `app/pages/`：历史回测 T0、复评中心、参数实验室
- `core/models.py`：数据模型
